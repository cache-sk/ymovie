namespace ymovie.view.catalogue {
	export class CatalogueMedia extends CatalogueItem<type.Type.Item> {
		...
		render() {
			...
			const playable = (this.data instanceof type.Type.PlayableSccItem && this.data.streamCount && this.data.streamCount > 0) 
				|| this.data instanceof type.Type.WebshareItem;
		...
		renderPoster() {
			const poster = this.data instanceof type.Type.SccItem ? this.data.posterThumbnail : this.data?.poster;
