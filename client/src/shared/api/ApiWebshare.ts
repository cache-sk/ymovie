namespace ymovie.shared {
	export class ApiWebshare {
		...
		async getSalt(username:string){
			...
			return await this.loadValue(ApiWebshare.PATH_SALT, body, "salt");
		}
		
		async getLogin(username:string, password:string, salt:string){
			const hash = ApiWebshare.hashPassword(password, salt);
			const body = `username_or_email=${encodeURIComponent(username)}&keep_logged_in=1&password=${hash}`;
			return await this.loadValue(ApiWebshare.PATH_LOGIN, body, "token");
		}
		
		async getUsername(token:string){
			const body = `wst=${encodeURIComponent(token)}`;
			return await this.loadValue(ApiWebshare.PATH_USER_DATA, body, "username");
		}
		...
		async getLink(ident:string, token:string){
			...
			return await this.loadValue(ApiWebshare.PATH_FILE_LINK, body, "link");
		}
	...
	t.hashPassword = function(password:string, salt:string){
